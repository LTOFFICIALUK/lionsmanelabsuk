...
const Home = () => {
  ...
  const featuredProducts = Object.entries(PRODUCTS).map(([slug, product]) => ({
    slug,
    ...product,
    startingPrice: getStartingPrice(product.variants, product.price)
  })).slice(0, 4);

  // Calculate average rating for featured products
  const averageRating = featuredProducts.reduce((acc, product) => {
